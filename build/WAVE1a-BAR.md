...
| Component testing | `renderToStaticMarkup` from `react-dom/server` under vitest. **Superseded (W-PEAK-01a-closure-2):** this row previously read "(proven in Wave 0)", which was an **unbacked claim**. Wave 0's proof was an orchestrator probe run in a `/tmp` scratch directory and never landed in this repository, so until the artifact existed the string appeared in this repo **only inside bar documents — this one and `build/WAVE1a-CLOSURE2-BAR.md` — never in a test**. That is the failure mode rule 8 exists to stop: a bar citing a proof that is not on disk. The in-repo proof is now `tests/invites/sign-up-form.test.ts` (added by W-PEAK-01a-closure-2), which renders the real `AuthForm` and asserts the invite field present when the flag is on and absent when it is off. **Do NOT add a test dependency** — no jsdom, no happy-dom, no @testing-library. |
