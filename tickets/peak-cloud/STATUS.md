...
| **PEAK-240** Beta invite redemption | **PARTLY DONE** — the DONE was withdrawn by the orchestrator | Server-side enforcement is **verified**: a blind adversarial critic drove **13 attack classes** through the real `auth.handler()` — every attempt refused, **zero user rows created** — and its own kill-mutation proved the attack tested the gate; the final-use race is proven by a 4-client concurrency test that the orchestrator killed personally (`expected [ {ok:true}, {ok:true} ] to have a length of 1 but got 2`). The conditional field is implemented (`auth-form.tsx:39`, rendered at :100, transported at :71). **Why it is not DONE:** bar criterion 240.A7 requires proof on disk that the field appears when the flag is on and is absent when it is off, and `grep -rn renderToStaticMarkup tests/` returns **zero** — the bar demanded an artifact while the unit's own prompt told it not to leave a test file, so the code shipped unproven and the ticket was marked DONE on the strength of the implementation rather than the artifact. Rule 6 and rule 8 both apply: the status is corrected and a follow-up wave adds the test |
